...
pub fn statement_body(i: Input) -> IResult<Statement> {
    ...
    let create_materialized_view = map_res(
        rule! {
            CREATE ~ ( OR ~ ^REPLACE )? ~ ( ASYNC | SYNC )? ~ MATERIALIZED ~ VIEW ~ ( IF ~ ^NOT ~ ^EXISTS )?
            ~ #dot_separated_idents_1_to_3
        ...
        |(
            ...
            Ok(Statement::CreateMaterializedView(
                CreateMaterializedViewStmt {
                    create_option,
                    sync_creation: opt_creation_mode
                        .is_none_or(|mode| mode.text().eq_ignore_ascii_case("sync")),
                    catalog,
                    database,
                    view,
                    cluster_by: opt_cluster_by.map(|(_, _, _, exprs, _)| ClusterOption {
                        cluster_exprs: exprs,
                    }),
