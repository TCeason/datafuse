...
stmt_with_user "show databases" "$TEST_U1_CONNECT"
stmt_with_user "create database db1;" "$TEST_U1_CONNECT"
stmt_with_user "create table db1.t1(id int);" "$TEST_U1_CONNECT"
stmt_with_user "insert into db1.t1 values(1);" "$TEST_U1_CONNECT"
stmt_with_user "select * from db1.t1;" "$TEST_U1_CONNECT"
stmt_with_user "select * from db_root.t1;" "$TEST_U1_CONNECT"
stmt_with_user "show databases" "$TEST_U1_CONNECT"
...
stmt_with_user "show databases" "$TEST_U2_CONNECT"
stmt_with_user "create database db2" "$TEST_U2_CONNECT"
stmt_with_user "create table db2.t2(id int);" "$TEST_U2_CONNECT"
stmt_with_user "insert into db2.t2 values(2);" "$TEST_U2_CONNECT"
stmt_with_user "show databases" "$TEST_U2_CONNECT"
stmt_with_user "select * from db2.t2;" "$TEST_U2_CONNECT"
stmt_with_user "select * from db1.t1;" "$TEST_U2_CONNECT"
stmt_with_user "select * from db2.t2;" "$TEST_U1_CONNECT"
...
stmt_with_user "show databases" "$TEST_U3_CONNECT"
stmt_with_user "create database db_u3" "$TEST_U3_CONNECT"
stmt_with_user "create table db_u3.t3(id int)" "$TEST_U3_CONNECT"
stmt_with_user "insert into db_u3.t3 values(3)" "$TEST_U3_CONNECT"
stmt_with_user "show databases" "$TEST_U3_CONNECT"
stmt_with_user "select * from db1.t1" "$TEST_U3_CONNECT"
stmt_with_user "select * from db2.t2" "$TEST_U3_CONNECT"
stmt_with_user "select * from db_root.t1" "$TEST_U3_CONNECT"
stmt_with_user "select * from db_u3.t3" "$TEST_U3_CONNECT"

echo "=== test u3 with role2 and role1 secondary roles all ==="
echo "grant role role1 to u3" | $BENDSQL_CLIENT_CONNECT
stmt_with_user "SET SECONDARY ROLES ALL; show databases" "$TEST_U3_CONNECT"
stmt_with_user "SET SECONDARY ROLES ALL; select * from db1.t1" "$TEST_U3_CONNECT"
stmt_with_user "SET SECONDARY ROLES ALL; select * from db2.t2" "$TEST_U3_CONNECT"
stmt_with_user "SET SECONDARY ROLES ALL; select * from db_root.t1" "$TEST_U3_CONNECT"
stmt_with_user "SET SECONDARY ROLES ALL; select * from db_u3.t3" "$TEST_U3_CONNECT"

echo "=== test u3(set role1) with role2 and role1 secondary roles none ==="
stmt_with_user "set role role1; SET SECONDARY ROLES NONE; show databases;" "$TEST_U3_CONNECT"
stmt_with_user "set role role1; SET SECONDARY ROLES NONE; select * from db1.t1" "$TEST_U3_CONNECT"
stmt_with_user "set role role1; SET SECONDARY ROLES NONE; select * from db2.t2" "$TEST_U3_CONNECT"
stmt_with_user "set role role1; SET SECONDARY ROLES NONE; select * from db_root.t1" "$TEST_U3_CONNECT"
stmt_with_user "set role role1; SET SECONDARY ROLES NONE; select * from db_u3.t3" "$TEST_U3_CONNECT"
