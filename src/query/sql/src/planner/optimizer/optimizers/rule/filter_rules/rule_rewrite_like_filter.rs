...
use crate::plans::{ConstantExpr, DummyTableScan, EvalScalar, FunctionCall, ScalarItem, Scan};
use crate::plans::Operator;
use crate::plans::RelOp;
use crate::{ColumnSet, ScalarExpr};
            ...
            match &scalar_expr {
                ScalarExpr::FunctionCall(func) if func.func_name == "like" => {
                    if let (
                        ...
                        ScalarExpr::ConstantExpr(c),
                    ) = (&func.arguments[0], &func.arguments[1])
                    {
                        if matches!(*left_col.column.data_type, DataType::String) {
                            if let Scalar::String(v) = &c.value {
                                if v == "%" {
                                    (true, ScalarExpr::ConstantExpr(ConstantExpr {
                                        span: None,
                ...
                ScalarExpr::FunctionCall(func) => {
                    ...
                    if fold {
                        (true, ScalarExpr::FunctionCall(FunctionCall {
                            span: func.span,
                        ...
                        (false, ScalarExpr::FunctionCall(FunctionCall {
                            span: func.span,
                            func_name: func.func_name.clone(),
                            params: func.params.clone(),
                            arguments,
