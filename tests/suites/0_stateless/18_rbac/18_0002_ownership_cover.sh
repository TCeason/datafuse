...
## create user
stmt "create user '${TEST_USER_NAME}' IDENTIFIED BY '$TEST_USER_PASSWORD'"
## create role
stmt 'create role `r_0002`'
stmt "GRANT ROLE 'r_0002' TO '${TEST_USER_NAME}'"
stmt "GRANT CREATE DATABASE, SUPER ON *.* TO ROLE 'r_0002'"
stmt "create ROLE r_0002_1"
stmt "create user 'owner1' IDENTIFIED BY 'password'"
stmt "grant ROLE r_0002_1 to user owner1"

stmt_with_user "set default role 'r_0002'" $TEST_USER_CONNECT

## database/table
echo "=== test db/table ==="
stmt_with_user "create database d_0002" $TEST_USER_CONNECT
stmt_with_user "create table d_0002.t(id int)" $TEST_USER_CONNECT
stmt_with_user "insert into d_0002.t values(200)" $TEST_USER_CONNECT
stmt_with_user "select * from d_0002.t" $TEST_USER_CONNECT

## stage
echo "=== test stage ==="
stmt_with_user 'create stage hello' $TEST_USER_CONNECT
stmt_with_user 'COPY INTO @hello from (select number from numbers(1)) FILE_FORMAT = (type = parquet)' $TEST_USER_CONNECT
stmt_with_user 'select * from @hello' $TEST_USER_CONNECT

## udf
echo "=== test udf ==="
stmt_with_user "create function a as (a) -> (a+1);" $TEST_USER_CONNECT
stmt_with_user "select a(1);" $TEST_USER_CONNECT
...
stmt_with_user "set default role 'r_0002_1'" $TEST_TRANSFER_USER_CONNECT
echo "=== test role r_0002_1 ==="
stmt_with_user "create table d_0002.t1(id int)" $TEST_TRANSFER_USER_CONNECT
stmt_with_user "select a(1);" $TEST_TRANSFER_USER_CONNECT
stmt_with_user 'select * from @hello' $TEST_TRANSFER_USER_CONNECT
stmt_with_user "select * from d_0002.t" $TEST_TRANSFER_USER_CONNECT

echo "=== test role r_0002 ==="
stmt_with_user "create table d_0002.t2(id int)" $TEST_TRANSFER_USER_CONNECT
stmt_with_user "select a(1);" $TEST_USER_CONNECT
stmt_with_user 'select * from @hello' $TEST_USER_CONNECT
stmt_with_user "select * from d_0002.t" $TEST_USER_CONNECT
...
stmt_with_user "set default role role1;" $USER_A_CONNECT
stmt_with_user "show roles;" $USER_A_CONNECT
stmt_with_user "create table db_a.t(id int);" $USER_A_CONNECT
stmt_with_user "create table db_a.t1(id int);" $USER_A_CONNECT
stmt_with_user "show tables from db_a" $USER_A_CONNECT
stmt_with_user "show databases" $USER_A_CONNECT
...
stmt_with_user "show tables from db_a;" $USER_A_CONNECT
stmt_with_user "show tables from db_a;" $USER_B_CONNECT

stmt "revoke role role1 from a;"
stmt "show grants for a ;"

stmt_with_user "show tables from db_a;" $USER_A_CONNECT
stmt_with_user "show tables from db_a;" $USER_B_CONNECT

stmt_with_user "drop table db_a.t1;" $USER_B_CONNECT
...
stmt_with_user "create database a" "$USER_U1_CONNECT"
stmt_with_user "create table a.t(id int)" "$USER_U1_CONNECT"
stmt_with_user "select name, owner from system.databases where name='a'" "$USER_U1_CONNECT"
stmt_with_user "select name, owner from system.tables where database='a'" "$USER_U1_CONNECT"
